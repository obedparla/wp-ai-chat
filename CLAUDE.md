...
- **SSE buffering when proxying streams in PHP**: reading an SSE stream via `fopen()`/`fread()` batches data into 8KB bursts — PHP's HTTP wrapper fills its internal buffer to the default 8192-byte chunk size before `fread()` returns. Fix: `stream_set_chunk_size($stream, 256)` + `stream_set_read_buffer($stream, 0)` (see `WPAIC_Chat::stream_from_provider`). Related: the provider must not forward OpenAI lifecycle events (`response.created`/`in_progress`/`completed`) wholesale — each embeds the full instructions + tools (~25KB); `WPAIP_Streamer::slim_event_payload` whitelists and slims forwarded events to only what the chatbot consumes.
