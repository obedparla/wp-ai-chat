...
**Status: implemented across rounds 1-3, 2026-06-10** — all items below (P0, P1, P2, round-2/3 addenda) are shipped.
...
- [x] **P0-1 [frontend, small] Fix cart-action replay: persist executed add_to_cart intents**
...
- [x] **P0-2 [frontend, medium] Fix variable-product add-to-cart (silent no-op) + live variation price**
...
- [x] **P0-3 [backend, medium] Stop asserting false store facts: zero-result search retry + shipping cross-check**
...
- [x] **P0-4 [cross, medium] Abuse controls: provider usage metering/caps + chatbot rate limiting + request caps**
...
- [x] **P0-5 [provider, small] Cost quick wins: prompt_cache_key, reasoning low, none→minimal, store=false**
...
- [x] **P0-6 [backend, small] Token diet: split model-facing vs frontend-facing tool payloads**
...
- [x] **P0-7 [frontend, small] Cut the 3-second send debounce to ~400ms**
...
- [x] **P0-8 [admin, small] Fix destructive saves: FAQ truncate-before-validate, CSV delete-before-import**
...
- [x] **P0-9 [frontend, medium] Scope Tailwind preflight — stop breaking host themes**
...
- [x] **P1-10 [backend, medium] Carry compact tool context across turns; ground ordinal references in card order**
...
- [x] **P1-11 [backend, small] Comparison trust: server-computed differences in compare_products + real attributes**
...
- [x] **P1-12 [frontend, small] Card rendering discipline: dedupe, suppress redundant carousels, cap picks, single card after add**
...
- [x] **P1-13 [backend, medium] Merchandising tools: coupons/what's-on-sale + cross-sells after add-to-cart**
...
- [x] **P1-14 [admin, small] Fix transcript data loss + attach conversations to handoff requests**
...
- [x] **P1-15 [admin, medium] Insights: summary cards, missed-search report, chat-to-cart attribution, scannable logs**
...
- [x] **P1-16 [backend, small] Humanize category and attribute labels everywhere**
...
- [x] **P1-17 [frontend, small] Mobile viewport fixes: 100dvh, body scroll lock, admin-bar offset, zero-price guard**
...
- [x] **P1-18 [frontend, medium] Proactive teaser bubble instead of auto-opening the full widget**
...
- [x] **P1-19 [frontend, medium] Skeleton product cards during product tool calls + unread badge on launcher**
...
- [x] **P1-20 [backend, small] Conversation-engine robustness: try/catch tool execution, error objects for null, quiet logs**
...
- [x] **P1-21 [provider, small] Provider hardening: HTTP timeout + retry, disconnect detection, grace-period signature check, friendly errors**
...
- [x] **P1-22 [admin, small] First-run onboarding checklist**
...
- [x] **P2-23 [backend, medium] Remove the legacy direct-OpenAI path + dead Settings API code**
...
- [x] **P2-24 [frontend, medium] Lazy-load the widget bundle**
...
- [x] **P2-25 [frontend, small] Inline error state for card add-to-cart failures (stop navigating away)**
...
- [x] **P2-26 [frontend, small] Accessibility pass**
...
- [x] **P2-27 [admin, small] Admin truthfulness bundle: unsaved-changes guard, index freshness wording, handoff badge, markdown transcripts**
...
- [x] **P2-28 [admin, small] Privacy + lifecycle hygiene: retention setting, IP anonymization, uninstall.php**
...
- [x] **NEW-A [backend, small] Search recall + grounding polish (verification findings)**
...
- [x] **NEW-B [provider, small] Provider admin: mask OpenAI key + surface cache hit rate**
...
- [x] **FIX-1 [backend, small, CRITICAL] Stop "Products shown (display order)" context leaking into replies**
...
- [x] **FIX-2 [backend, small] Humanize slug-like category NAMES + importer**
...
- [x] **FIX-3 [backend, small] Phrase-level synonym recall ("running shoes" → sneakers)**
...
- [x] **FIX-4 [backend, small] Clarifying questions must show candidate cards**
...
- [x] **FIX-5 [admin, tiny] Legacy handoff fallback renders empty role chip**
...
- [x] **FIX-6 [provider, tiny] Freemius API token field echoes saved value**
  Same exposure as the OpenAI key field had — apply the same masked-placeholder pattern.

- [x] **P1-11b [frontend, small] Render the richer comparison payload**
