...
- Replies render in stream-arrival order with no buffering: product cards / comparison / checkout button mount as soon as their tool outputs arrive (above the text), and the reply text streams live below them — content only appends at the bottom, so nothing jumps or flickers; pipeline tuned for per-delta delivery (provider forwards only slim consumed events, chatbot reads the provider stream un-buffered)
...
### Admin Pages
- Analytics: a charted dashboard of what the chatbot is driving — revenue driven (gross, from completed orders the bot's cart-add touched), share of store revenue, orders, conversion rate, AOV (bot vs store), items added, conversations/avg messages, a conversion funnel (conversations → products shown → cart → checkout → order) with drop-off, a revenue-over-time area chart, top products / top searches / missed-search leaderboards, and a day×hour busiest-times heatmap. Range switcher (7d / 30d / 90d / all) reloads server-side; hero KPIs show period-over-period deltas; transient-cached per range. React island skinned with the admin design tokens; degrades to a chat-only view when WooCommerce is inactive and to an empty state on fresh installs. Order attribution tags the WooCommerce session on bot cart-adds and records an `order_completed` event once on payment (HPOS-safe, idempotent).
